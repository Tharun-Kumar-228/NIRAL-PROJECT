...
const StartWorkMapPage = ({ route, navigation }) => {
  ...
  const handleConfirm = async () => {
    try {
      await axios.put(`http://${IPADD}:5000/api/works/start/${workId}`, {
      ...
      console.error(err);
      Alert.alert('Error starting work');
    }
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* Confirm Button */}
      <Button title="Confirm Start" onPress={handleConfirm} />
    </View>
  );
};
...
export default StartWorkMapPage;
